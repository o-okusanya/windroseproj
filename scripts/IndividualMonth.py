...
class PipelineMonthIndividual(WindPlotIndividual):
    def runmonth(self, stations):
        ...
        for station in stations:
            logger.info(f"[month] {sd} -> {ed} for {station}")
            ...
            try:
                ...
                self.plot(
                    grouped,
                    fname=f"wind_rose_{strmonth}_{year}_{station}",
                    sd=sd,
                    ed=ed
                )
...
if __name__ == "__main__":
    ...
    logger.info(f"Running Month for {stations}")
    PipelineMonthIndividual().runmonth(stations)